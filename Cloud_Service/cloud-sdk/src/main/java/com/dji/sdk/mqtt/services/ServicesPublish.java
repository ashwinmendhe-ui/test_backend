...
public class ServicesPublish {
    private static final Logger log = LoggerFactory.getLogger(ServicesPublish.class);
    @Resource
    ...
    public <T> TopicServicesResponse<ServicesReplyData<T>> publish(
        ...
        if (Objects.nonNull(replyReceiver.getInfo())) {
            Object info = replyReceiver.getInfo();
            reply.setOutput(info instanceof String ? mapper.convertValue(info, clazz) :
                    mapper.convertValue(info.toString(), clazz));
        }
        if (Objects.nonNull(replyReceiver.getOutput())) {
            Object output = replyReceiver.getOutput();
            reply.setOutput(output instanceof String ? mapper.convertValue(output, clazz) :
                    mapper.convertValue(output.toString(), clazz));
        }
        return response.setData(reply);
    }
